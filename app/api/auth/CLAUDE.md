...
| `can_evolve` | Threader | Allows the user to access `/thread` and submit change requests to Claude Code. Granted/revoked by admins via `/admin`. |
...
- `hasThreadPermission(userId)` — true if user has `admin` or `can_evolve` role
