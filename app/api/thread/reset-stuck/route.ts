// app/api/thread/reset-stuck/route.ts
...
export async function POST(request: Request) {
  ...
  if (record.status !== 'accepting' && record.status !== 'fixing-types') {
    return Response.json(
      {
        error: `Thread is not stuck (status: ${record.status}). Only 'accepting' and 'fixing-types' threads can be force-reset.`,
      },
      { status: 409 },
    );
  }
  ...
  if (!fs.existsSync(ndjsonPath)) {
    return Response.json({ error: 'Thread log not found on disk' }, { status: 404 });
  }
  ...
  appendSessionEvent(ndjsonPath, {
    ...
    subtype: 'error',
    message:
      `❌ Thread force-reset by user. The ${record.status === 'accepting' ? 'deploy' : 'type-fix'} ` +
