// app/api/thread/from-branch/route.ts
// Start an thread on an existing git branch (e.g. from an external contributor).
...
/** JSON body for POST /thread/from-branch */
export interface ThreadFromBranchBody {
  branchName: string; // Name of the existing local git branch to attach a session to. Must not contain a slash.
}

/**
 * Start an thread on an existing branch
