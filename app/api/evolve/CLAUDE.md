...
User types change request on /evolve page
  ...
  → User clicks Accept → POST /api/evolve/manage { action: "accept" }
      ...
      → blue/green deploy (production): bun install in worktree → session branch becomes new prod as-is (no merge commit; Gate 1 guarantees it already contains parentBranch)
          ...
          → sibling sessions using legacy git-config parentage are reparented to the session branch; branch-marker parentage remains immutable, and marker-mode parent resolution dynamically falls back to current production when the original parent has been deployed
          ...
          → install.sh starts the new production server through `bun run process <branch> start --prod`, health-checks it, sets primordia.productionBranch + productionHistory in git config, and switches traffic; the proxy owns no app server processes
...
| session branch behind parent → current code/data | `POST /api/evolve/upstream-sync` merges the parent/prod branch, runs `bun install` in the updated worktree, creates a production DB snapshot via `VACUUM INTO`, then asks the running preview server to close and hot-swap its SQLite DB before reopening on the next DB access |
