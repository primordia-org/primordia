// app/api/server/kill-restart/route.ts
// Restarts a thread's preview server through the shared process manager.
//
// POST
...
/** JSON body for POST /server/kill-restart */
export interface ServerKillRestartBody {
  threadId: string; // The thread ID (git branch name) whose preview dev server should be restarted.
}

/**
 * Restart a thread's preview dev server
 * @description Uses the process manager to kill and restart the thread's preview dev server process.
 * @tag Server
 * @body ServerKillRestartBody
 */
