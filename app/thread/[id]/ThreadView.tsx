...
function getAttachmentUrl(sessionId: string, name: string): string {
  return withBasePath(`/api/thread/attachment/${encodeURIComponent(sessionId)}?file=${encodeURIComponent(name)}`);
}
...
export default function ThreadView({
  ...
  useEffect(() => {
    fetch(withBasePath('/api/thread/models'))
      .then((r) => r.json())
  ...
  const refreshDiffSummary = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        withBasePath(`/api/thread/diff-summary?threadId=${sessionId}`),
        { cache: "no-cache" },
      );
  ...
  async function handleAbort() {
    ...
    try {
      const res = await fetch(withBasePath('/api/thread/abort'), {
        method: 'POST',
  ...
  async function handleUpstreamSync() {
    ...
    try {
      const res = await fetch(withBasePath('/api/thread/upstream-sync'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function handleForceReset(targetSessionId: string) {
    ...
    try {
      const res = await fetch(withBasePath('/api/thread/reset-stuck'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
