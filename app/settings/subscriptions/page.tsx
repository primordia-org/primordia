...
import CredentialsSettingsClient from "./CredentialsSettingsClient";
import ChatGptSubscriptionSettingsClient from "./ChatGptSubscriptionSettingsClient";

export function generateMetadata(): Metadata {
  return {
    title: buildPageTitle("Account Settings — Subscriptions"),
    description: "Connect Claude.ai and ChatGPT subscriptions for evolve requests.",
  };
}

export default async function SettingsSubscriptionsPage() {
  const user = await getSessionUser();
  if (!user) redirect("/login?next=/settings/subscriptions");
  ...
  return (
    <main className="flex flex-col w-full max-w-5xl mx-auto px-4 py-6 min-h-dvh">
      ...
      <div className="flex flex-col lg:flex-row gap-4 lg:gap-8 items-start mt-2">
        <SettingsSubNav currentTab="subscriptions" />
        <div className="flex-1 min-w-0 flex flex-col gap-6">
          <CredentialsSettingsClient />
