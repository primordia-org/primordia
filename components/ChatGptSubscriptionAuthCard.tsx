...
import { clearSecret, decryptStoredSecretPayload, setSecret } from "@/lib/secrets-client";
import { generateScramble, useDecryptEffect } from "@/lib/use-decrypt-effect";
...
interface ChatGptSubscriptionAuthCardProps {
...
export function ChatGptSubscriptionAuthCard({
  initialCiphertext,
  title = "ChatGPT",
  description,
  connectedMessage,
  startButtonLabel = "Sign in with ChatGPT",
  reconnectButtonLabel = "Sign in again",
  showStoredCredentials = true,
  allowDisconnect = true,
  dataIdPrefix = "chatgpt-subscription",
  eventPrefix = "settings/subscriptions/chatgpt",
  className = "",
}: ChatGptSubscriptionAuthCardProps) {
  ...
  async function startAuth() {
    ...
    try {
      ...
      setDeviceFlow(next);
      trackEvent(`${eventPrefix}-started/v1`, {});
  ...
  async function pollChatGpt(flow: DeviceFlowState) {
    try {
      ...
      if (res.ok && data.status === "connected") {
        ...
        trackEvent(`${eventPrefix}-connected/v1`, {});
        return;
      }
  ...
  async function disconnect() {
    ...
    try {
      ...
      setCredentialsRevealed(false);
      setJustConnected(false);
      trackEvent(`${eventPrefix}-disconnected/v1`, {});
    } catch {
  ...
  const showCredentialsBlock = showStoredCredentials && (isDecrypting || credentialsRevealed);
  const hasCredentials = Boolean(credentials || hasStoredCredentials);

  return (
    <div className={`rounded-xl border border-gray-700 bg-gray-900 p-5 flex flex-col gap-5 ${className}`}>
      ...
      {showStoredCredentials && credentials && (
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-3">
              {credentialsRevealed && (
                <button
                  type="button"
                  data-id={`${dataIdPrefix}/copy-credentials`}
              ...
              <button
                type="button"
                data-id={`${dataIdPrefix}/toggle-visibility`}
                onClick={() => {
      ...
      {deviceFlow && (
        <div className="flex flex-col gap-3">
          <div className="flex flex-col gap-1.5">
            <label className="text-xs text-gray-400 font-medium" htmlFor={`${dataIdPrefix}-device-code`}>
              One-time code
            </label>
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
              <div
                id={`${dataIdPrefix}-device-code`}
              ...
              <button
                type="button"
                data-id={`${dataIdPrefix}/copy-code`}
      ...
      {justConnected && connectedMessage && (
        <p className="rounded-lg border border-emerald-800/60 bg-emerald-950/30 px-3 py-2 text-xs text-emerald-200">
          {connectedMessage}
        </p>
      )}
      {error && <p className="text-sm text-red-400">{error}</p>}

      {(hasCredentials || !deviceFlow) && (
        <div className="flex items-center gap-2">
          {allowDisconnect && hasCredentials && (
            <button
              data-id={`${dataIdPrefix}/disconnect`}
          ...
          {!deviceFlow && (
            <button
              data-id={`${dataIdPrefix}/start-auth`}
              onClick={() => void startAuth()}
              disabled={busy}
              className={`${allowDisconnect && hasCredentials ? "flex-1" : "w-full"} px-4 py-2 rounded-lg text-sm font-medium bg-sky-600 hover:bg-sky-500 disabled:bg-sky-900 text-white transition-colors disabled:cursor-not-allowed`}
            >
              {busy ? "Starting…" : hasCredentials ? reconnectButtonLabel : startButtonLabel}
            </button>
          )}
        </div>
      )}
