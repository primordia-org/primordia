...
| `branch.{name}.*` | `branch.feature-x.parent` | Legacy parent branch metadata; still written while fork-marker commit trailer tracking (`lib/branch-parent.ts`) is validated via a user-toggleable source setting |
