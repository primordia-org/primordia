export const PROGRESS_MONITOR_PROMPT = [
  ...
  'Every run starts with one active step named `Make a plan`, unless this follow-up prompt explicitly says a previous progress task list is incomplete; in that case continue from the provided current task list state.',
  ...
  'Do not rewrite terminal steps. If you realize more work is needed after all listed steps are terminal and details are appearing under the virtual `Wrap-up` step, use `bun run progress plan insert \'[...]\'` to append the newly discovered work; the first inserted step becomes active.',
