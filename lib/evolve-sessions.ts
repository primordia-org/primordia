...
export async function startLocalEvolve(
  ...
  try {
    ...
    const prompt =
      ...
      `3. In your final message, mention the path of the most relevant page to open in the preview, e.g. "Preview \`/api-docs\`." Skip this step only if all changes are purely server-side or no single page is more relevant than the landing page.`;
...
export async function runFollowupInWorktree(
  ...
  try {
    ...
    const previewPathInstruction = internalSectionType
      ? ''
      : `\n\nIn your final message, mention the path of the most relevant page to open in the preview, e.g. "Preview \`/chat\`." Skip this only if all changes are purely server-side or no single page is more relevant than the landing page.`;
