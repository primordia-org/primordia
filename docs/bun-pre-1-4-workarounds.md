...
- **Bun 1.4 result:** Keep the direct CLI-file invocation. On Bun 1.4.0, `bun run --bun next dev --turbopack` still launches a `node .../node_modules/.bin/next dev` CLI process, while the direct form launches the Next CLI as `bun` and then the dev server as `bun .../next/dist/server/lib/start-server.js`. More importantly, `bun run --bun next start` still runs the listening production server as `node .../node_modules/.bin/next start`; `bun --bun ./node_modules/next/dist/bin/next start` keeps the listening production process as `bun`. Both forms can build successfully, serve app routes that touch SQLite, and be found by the process manager through the assigned port, but the shorter package-bin form would reintroduce the misleading Node runtime in production process listings/status. The current `package.json` scripts remain the safer, explicit choice.
- **Investigation prompt:** Re-run the comparison after future Bun upgrades. Confirm process runtimes, `bun:sqlite` availability in server/build workers, and process-manager status detection before deciding whether to simplify `package.json`.
