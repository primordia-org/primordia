// scripts/worktree-session-daemon.ts
// Worktree session lifecycle daemon for Primordia production and preview servers.
//
// Listens on WORKTREE_DAEMON_SOCKET (default /tmp/primordia-worktree-daemon-<port>.sock).
// The public reverse proxy delegates /_proxy/* requests here and keeps normal
// traffic routing in scripts/reverse-proxy.ts.
//
// Responsibilities:
//   - Start the production Next.js server on boot if needed.
//   - Handle POST /_proxy/prod/spawn for blue/green deploy slot activation.
//   - Capture production server logs for GET /_proxy/prod/logs.
//   - Start, restart, stop, idle-reap, and stream logs for preview dev servers.
//   - Run automatic disk cleanup for old non-production worktrees.
//
// The daemon still reads branch ports and production branch metadata from git
// config, matching the proxy's routing source of truth.
...
function logCrashBoundary(label: string, err: unknown): void {
  console.error(`[daemon] ${label}:`, errorMessage(err));
}
  ...
  if (prodBranch) {
    ...
    if (!port) {
      ...
      try {
        ...
        console.error(`[daemon] could not assign port for '${prodBranch}': ${errorMessage(err)}`);
        return;
      }
      try {
        ...
        console.log(`[daemon] auto-assigned port :${port} to branch '${prodBranch}'`);
        branchPort[prodBranch] = port;
      } catch (err) {
        console.warn(`[daemon] could not persist port for '${prodBranch}': ${(err as Error).message}`);
      }
    }
    const portChanged = port !== upstreamPort;
    if (portChanged) {
      console.log(`[daemon] upstream port: ${upstreamPort} → ${port} (PROD branch: ${prodBranch})`);
      upstreamPort = port;
    }
...
async function killPortOwner(port: number): Promise<void> {
  ...
  console.log(`[daemon] sent SIGTERM to ${pids.length} process(es) on :${port}`);
  ...
  while (Date.now() < deadline) {
    ...
    if (!stillInUse) {
      console.log(`[daemon] port :${port} is now free`);
      return;
    }
  }
  ...
  try {
    ...
    if (remainingPids.length > 0) {
      console.log(`[daemon] sent SIGKILL to ${remainingPids.length} process(es) on :${port} after timeout`);
    }
...
async function startPreviewServer(
  ...
  console.log(`[daemon] starting preview server for session ${sessionId} on :${info.port} in ${info.worktreePath}`);
  ...
  if (previewProcesses.get(sessionId) !== entry) {
    console.log(`[daemon] aborting preview start for session ${sessionId} (superseded by concurrent operation)`);
    return entry;
  }
  ...
  const appendLog = (text: string) => {
    ...
    if (entry.status === 'starting' && text.includes('Ready')) {
      ...
      console.log(`[daemon] preview server ready for session ${sessionId} on :${info.port}`);
      const waiters = entry.startWaiters.splice(0);
      for (const w of waiters) {
  ...
  proc.on('close', (code) => {
    ...
    console.log(`[daemon] preview server stopped for session ${sessionId} (code ${code ?? 'unknown'})`);
    for (const sub of entry.logSubscribers) {
...
function stopPreviewServer(sessionId: string): void {
  ...
  console.log(`[daemon] stopping preview server for session ${sessionId}`);
  entry.status = 'stopped';
  ...
  for (const [sessionId, entry] of previewProcesses.entries()) {
    if (entry.lastActivityMs < cutoff) {
      if (entry.status === 'stopped') {
        ...
        console.log(`[daemon] stopping idle preview server ${sessionId} (${previewInactivityMin} min inactivity)`);
 ...
 * This keeps production server lifecycle in the daemon while the reverse proxy
 * only routes traffic to the active branch port.
 */
async function startProdServerIfNeeded(): Promise<void> {
  ...
  try {
    ...
    console.log(`[daemon] production server already running on :${upstreamPort}`);
    return;
  } catch {
  ...
  try {
    ...
    console.log(`[daemon] worktree lookup for '${currentProdBranch}': found=${prodPath ?? 'none'}`);
    if (!prodPath) {
      // Diagnostic: log all worktrees to aid debugging
      console.log(`[daemon] git worktree list output:\n${wtOut}`);
    }
  } catch (err) {
    console.warn(`[daemon] git worktree list failed: ${(err as Error).message}`);
  }
  ...
  if (!prodPath) {
    ...
    try {
      ...
      if (headBranch === currentProdBranch) {
        console.log(`[daemon] falling back to MAIN_REPO (${MAIN_REPO}) for branch '${currentProdBranch}'`);
        prodPath = MAIN_REPO;
      }
    } catch { /* ignore */ }
  }
  ...
  if (!prodPath) {
    console.warn(`[daemon] cannot start prod server: no worktree for branch '${currentProdBranch}'`);
    return;
  }

  console.log(`[daemon] starting production server (${currentProdBranch}) on :${upstreamPort} in ${prodPath}`);
  ...
  try {
    ...
    console.error(`[daemon] production server spawn failed: ${errorMessage(err)}`);
    return;
  }
  ...
  server.on('exit', (code) => {
    if (prodServerEntry?.process === server) {
      console.log(`[daemon] production server exited (code ${code ?? 'unknown'})`);
...
async function handleProdSpawn(
  ...
  try {
    ...
    try {
      ...
      console.log(`[daemon] auto-assigned port :${port} to branch '${branch}' for prod spawn`);
    } catch (err) {
      clientRes.writeHead(500, { 'content-type': 'application/json' });
  ...
  try {
    ...
    console.log(`[daemon] prod slot activated: ${branch} on :${port} (old :${oldPort}; old server will self-terminate)`);
    _success('Web traffic is now being directed to this server');
    sendDone(true);
  } catch (err) {
    if (_spinTimer) { clearInterval(_spinTimer); _spinTimer = null; }
    const msg = err instanceof Error ? err.message : String(err);
    console.error('[daemon] handleProdSpawn error:', msg);
    sendDone(false, msg);
  }
}
...
function forwardToPort(
  ...
  try {
    ...
    console.error(`[daemon] could not create upstream request on port ${port}:`, errorMessage(err));
  ...
  upstreamReq.on('error', (err) => {
    console.error(`[daemon] upstream error on port ${port}:`, err.message);
...
async function handlePreviewRequest(
  ...
  if (!entry || entry.status === 'stopped') {
    ...
    if (info.port === upstreamPort) {
      console.warn(`[daemon] refusing preview for session ${sessionId}: worktree is the current production server (port :${info.port})`);
...
function handleWsUpgrade(rawSocket: net.Socket, reqBuf: Buffer): void {
  rawSocket.on('error', (err) => {
    console.error('[daemon] client socket error during WS upgrade:', err.message);
  });
  ...
  upstreamSocket.on('connect', () => {
    ...
    upstreamSocket.once('data', (firstChunk: Buffer) => {
      ...
      if (!firstLine.startsWith('HTTP/1.1 101') && !firstLine.startsWith('HTTP/1.0 101')) {
        console.error(`[daemon] WS upstream on port ${targetPort} did not return 101`);
  ...
  upstreamSocket.on('error', (err) => {
    console.error(`[daemon] WS upstream error on port ${targetPort}:`, err.message);
    ...
    try {
      ...
      if (isWsUpgrade) {
        ...
        internal.on('error', (err) => {
          console.error('[daemon] internal handler connection error:', err.message);
          if (!rawSocket.destroyed) rawSocket.destroy();
        });
      }
    } catch (err) {
...
try { fs.unlinkSync(DAEMON_SOCKET_PATH); } catch { /* socket did not exist */ }
httpHandler.listen(DAEMON_SOCKET_PATH, () => {
  try { fs.chmodSync(DAEMON_SOCKET_PATH, 0o600); } catch { /* best-effort */ }
  console.log(`[daemon] listening on ${DAEMON_SOCKET_PATH}`);
  console.log(`[daemon] main repo: ${MAIN_REPO}`);
  console.log(`[daemon] worktrees: ${WORKTREES_DIR}`);
});
...
process.on('SIGTERM', () => {
  ...
  // Close the daemon HTTP socket.
  // Belt-and-suspenders: force exit after 5 s if connections don't drain.
  httpHandler.close(() => { try { fs.unlinkSync(DAEMON_SOCKET_PATH); } catch { /* already gone */ } process.exit(0); });
