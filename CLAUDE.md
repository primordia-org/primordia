...
| AI API | Anthropic SDK (`@anthropic-ai/sdk`) | Routes through exe.dev LLM gateway by default; users may override with stored billing-source secrets. Secrets stay encrypted in SQLite; each credential row has its own server ECDH keypair, and evolve workers read the selected ciphertext from SQLite and receive a `PRIMORDIA_DECRYPTION_KEY` derived from the browser's local ECDH secret and that row's server ECDH secret, avoiding browser decrypt-and-resend for worker auth. |
...
| Credentials management | ✅ Live | Account Settings includes unified Billing sources (`/settings`) and Presets (`/settings/presets`); users can connect Claude.ai credentials and ChatGPT subscription OAuth credentials, store API keys encrypted, and define evolve presets that bundle billing source + harness + model; evolve sessions pass only the local ECDH public key to the server, which derives `PRIMORDIA_DECRYPTION_KEY` using the selected credential row's server ECDH keypair; workers read and decrypt the selected SQLite ciphertext by user/auth source; evolve session ChatGPT auth failures render an inline re-login prompt that can reconnect the subscription without leaving the session page |
