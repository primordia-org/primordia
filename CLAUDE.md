...
|---|---|---|
| Evolve mode | ✅ Live | "Propose a change" in the hamburger opens a draggable/dockable floating dialog; `/thread` is the standalone thread creation page; before any thread worktree is deleted, its `.primordia-session.ndjson` log is saved as a gzip archive under `PRIMORDIA_DIR/past-sessions` when present |
| Local evolve pipeline | ✅ Live | git worktree → Claude Agent SDK → local preview → accept/reject |
| Evolve follow-up requests | ✅ Live | Chain multiple Claude passes on the same branch; form appears when the thread is ready; draft text persists across refreshes per thread |
...
| Evolve draft persistence | ✅ Live | Initial request drafts in `/thread` and the floating Propose-a-change dialog share a local timestamped draft; follow-up drafts are saved per thread until submitted; drafts older than one year are garbage-collected |
...
| Thread from existing branch | ✅ Live | Threads page shows "+ thread" next to branches with no active thread; evolvers can attach the full AI preview pipeline to any pre-existing local branch; the current branch is shown with its descendant tree even when it is outside production ancestry |
...
| Credentials management | ✅ Live | Account Settings includes unified Billing sources (`/settings`), Presets (`/settings/presets`), and a Primordia CLI helper (`/settings/cli`) for creating/revoking expiring `PRIMORDIA_CLI_KEY` values that wrap the browser-held AES JWK; users can connect Claude.ai credentials and ChatGPT subscription OAuth credentials, store API keys encrypted, and define evolve presets that bundle billing source + harness + model; thread ChatGPT auth failures render an inline re-login prompt that can reconnect the subscription without leaving the thread page |
| Credentials management | ✅ Live | Account Settings includes unified Billing sources (`/settings`), Presets (`/settings/presets`), and a Primordia CLI helper (`/settings/cli`) for creating/revoking expiring `PRIMORDIA_CLI_KEY` values that wrap the browser-held AES JWK; revoked CLI keys keep a non-secret history row with the encrypted AES wrapper secret cleared so terminal errors can distinguish revoked vs unknown keys; users can connect Claude.ai credentials and ChatGPT subscription OAuth credentials, store API keys encrypted, and define evolve presets that bundle billing source + harness + model; evolve session ChatGPT auth failures render an inline re-login prompt that can reconnect the subscription without leaving the session page |
...
| Process status CLI | ✅ Live | `bun run primordia status` lists the git-configured production branch, reverse proxy status, git worktrees sorted by assigned port, active Next.js server process state/env/PID/child count, and active agent worker PIDs without querying the reverse proxy; JSON output exposes `productionBranch`, `reverseProxy`, `servers`, and `agents` arrays; supports table and `--json` output; `bun run primordia start [--dev|--prod] [--json] [--worktree <worktreename>]`, `stop [--json] [--worktree <worktreename>]`, and `restart [--dev|--prod] [--json] [--worktree <worktreename>]` manage assigned-port worktree servers directly and append stdout/stderr to `.primordia-next-server.log`; `bun run primordia logs [--follow] [--json] [--worktree <worktreename>]` reads that log; `bun run primordia publish [--json] [--worktree <worktreename>]` health-checks the target branch's assigned port before marking it as `primordia.productionBranch`; the thread Web Preview logs stream over SSE from `/api/server/logs`, and restart controls/reverse proxy lazy preview spawning use this process-manager layer instead of reverse-proxy-owned preview processes |
| Thread CLI commands | ✅ Live | `bun run primordia create "request"` creates a thread and runs the initial agent turn from the terminal; `bun run primordia followup "request"` continues the thread identified by the current worktree; `bun run primordia update` applies parent/prod updates to the thread identified by the current worktree (or `--worktree`); `bun run primordia accept` and `bun run primordia reject` accept/deploy or discard the thread identified by the current worktree (or `--worktree`); CLI callers select users/presets with flags and must pass a revokable `PRIMORDIA_CLI_KEY` from `/settings/cli` for create/followup/accept commands; the CLI resolves that key to the worker-only AES env internally; billing source, harness, model, evolve permission checks, and accept/reject behavior live in `lib/threads.ts` (`createThread`/`followupThread` accept preset IDs and resolve preset details internally; `manageThread` is shared by the API route and CLI) so the CLI never imports API route modules directly. |
| Thread CLI commands | ✅ Live | `bun run primordia create "request"` creates a thread from the terminal, waits for setup to spawn the independent agent worker, and returns with the session ID while the worker continues; `bun run primordia followup "request"` continues the thread identified by the current worktree and also returns after spawning the independent worker; CLI callers select users/presets with flags and must pass a revokable `PRIMORDIA_CLI_KEY` for create/followup/accept commands; billing source, harness, model, and evolve permission checks live in `lib/threads.ts` (`createThread`/`followupThread` accept preset IDs and resolve preset details internally) so the CLI never imports API route modules directly. |
| Thread CLI commands | ✅ Live | `bun run primordia create "request"` creates a thread from the terminal, waits for setup to spawn the independent agent worker, and returns with the new worktree path for easy `cd`; `bun run primordia followup "request"` continues the thread identified by the current worktree and also returns after spawning the independent worker; CLI callers select users/presets with flags and must pass a revokable `PRIMORDIA_CLI_KEY` for create/followup/accept commands; billing source, harness, model, and evolve permission checks live in `lib/threads.ts` (`createThread`/`followupThread` accept preset IDs and resolve preset details internally) so the CLI never imports API route modules directly. |
| Process status CLI | ✅ Live | `bun run primordia status` lists the git-configured production branch, reverse proxy status, git worktrees sorted by assigned port, active Next.js server process state/env/PID/child count, and active agent worker PIDs without querying the reverse proxy; JSON output exposes `productionBranch`, `reverseProxy`, `servers`, and `agents` arrays; supports table and `--json` output; `bun run primordia start [--dev|--prod] [--json] [--worktree <worktreename>]`, `stop [--json] [--worktree <worktreename>]`, and `restart [--dev|--prod] [--json] [--worktree <worktreename>]` manage assigned-port worktree servers directly and append stdout/stderr to `.primordia-next-server.log`; `bun run primordia logs [--follow] [--json] [--worktree <worktreename>]` reads that log; `bun run primordia publish [--json] [--worktree <worktreename>]` health-checks the target branch's assigned port before marking it as `primordia.productionBranch`; `bun run primordia copydb [--json] [--worktree <worktreename>]` VACUUM-copies the production SQLite DB into the target worktree, defaulting to the worktree containing the current directory; the thread Web Preview logs stream over SSE from `/api/server/logs`, and restart controls/reverse proxy lazy preview spawning use this process-manager layer instead of reverse-proxy-owned preview processes |
| Process status CLI | ✅ Live | `bun run primordia status` lists reverse proxy status plus git worktrees sorted by assigned port, active Next.js server process state/env/PID/child count, and active agent worker PIDs without querying the reverse proxy; JSON output exposes `reverseProxy`, `servers`, and `agents` arrays; supports table and `--json` output; `bun run primordia start [--dev|--prod] [--json] [--worktree <worktreename>]`, `stop [--json] [--worktree <worktreename>]`, and `restart [--dev|--prod] [--json] [--worktree <worktreename>]` manage assigned-port worktree servers directly and append stdout/stderr to `.primordia-next-server.log`; `bun run primordia logs [--follow] [--json] [--worktree <worktreename>]` reads that log; the thread Web Preview logs stream over SSE from `/api/server/logs`, and restart controls/reverse proxy lazy preview/prod spawning use this process-manager layer instead of reverse-proxy-owned app processes; the proxy refuses to stop preview entries that resolve to current production and lazily starts production if it is accessed while down |
...
| Server health (/admin/server-health) | ✅ Live | Admin-only; disk and memory usage with visual bars; oldest non-prod worktree cleanup; scheduled CPU/memory leak diagnostics capture writes `leak-diagnostics/latest.md`, alerts admins, and can launch a thread to investigate/fix from Server Health |
