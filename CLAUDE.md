...
| Evolve CLI commands | ✅ Live | `bun run primordia create "request"` creates an evolve thread and runs the initial agent turn from the terminal; `bun run primordia followup "request"` continues the evolve thread identified by the current worktree; compatibility aliases are available as `bun run primordia evolve create|followup`; CLI callers select users/presets/models with flags and pass the browser-held AES JWK through `PRIMORDIA_AES_KEY` when a stored billing source must be decrypted; shared create-session behavior lives in `lib/evolve-create.ts` so the CLI never imports API route modules directly. |
