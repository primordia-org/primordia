...
| CLI framework | Internal tiny command framework (`lib/tiny-command/common.ts`, `lib/tiny-command/cli.ts`, `lib/tiny-command/rest.ts`) | Organizes `bun run primordia` subcommands, detailed handwritten-style help, argument/option parsing, JSON automation output, generated bash completion with dynamic completion hooks, an injectable `CommandContext` (`{ process, console }`) for command handlers, and Primordia Core route-action metadata/REST execution for exposing CLI-shaped commands through API endpoints |
...
| Thread CLI commands | ✅ Live | Agentic coding commands live under `bun run primordia thread`: `thread create "request"` creates a thread and returns the new worktree path for easy `cd`; `thread followup "request"`, `thread logs [--lines count] [--start line] [--follow] [--json]`, `thread update`, `thread accept`, and `thread reject` operate on the current thread resolved from cwd, without an explicit worktree override. `thread logs` renders `.primordia-session.ndjson` as human-readable console text by default and emits raw NDJSON events with `--json`. Create/followup support repeatable `--attach/-a <file>` attachments. `bun run primordia preferences get|set` reads and updates per-user thread defaults (preferred preset, fallback harness/model, and caveman mode/intensity). CLI callers select presets with flags and must pass a revokable `PRIMORDIA_API_KEY` for a `cli` client from `/settings/api-keys` for all user-scoped CLI commands; the CLI resolves the acting user and worker-only AES env from that key internally; billing source, harness, model, thread permission checks, and accept/reject behavior live in `lib/threads.ts` (`createThread`/`followupThread` accept preset IDs and resolve preset details internally; `manageThread` is shared by Core route-action handlers and CLI) so the CLI never imports API route modules directly. |
| Thread server CLI commands | ✅ Live | Current-thread process commands live under `bun run primordia server`: `server status [--follow] [--json]`, `server start [--dev|--prod] [--json]`, `server stop [--json]`, `server restart [--dev|--prod] [--json]`, `server logs [--lines count] [--start line] [--follow] [--json]`, `server publish [--json]`, and `server copydb [--json]`; all resolve the current thread from cwd and append/read `.primordia-next-server.log` through the shared process-manager layer. The CLI treats `--json` as machine-formatted output rather than necessarily one JSON document; log commands emit NDJSON when `--json` is set, wrapping plain text log lines in JSON objects. The thread Web Preview logs stream through Primordia Core `/api/core/server/[threadId]/logs` with visibility-aware `start` cursor resubscription, and restart controls/reverse proxy lazy preview/prod spawning use this process-manager layer instead of reverse-proxy-owned app processes; the proxy refuses to stop preview entries that resolve to current production and lazily starts production if it is accessed while down. Scheduled job commands live under `bun run primordia jobs`: `jobs run`, `jobs run-one <job>`, `jobs restart`, `jobs logs [-n count] [--start line] [-f]`, and `jobs schedule list|get|set`; intervals are stored in git config under `primordia.jobs.*IntervalMs`, and the systemd-facing Primordia service-supervisor keeps the dedicated jobs daemon alive alongside the reverse proxy. `bun run primordia systemd service-supervisor restart`, `bun run primordia reverse-proxy restart`, and `bun run primordia reverse-proxy logs [-n count] [--start line] [-f]` manage the supervisor and reverse proxy with clearer boundaries. Primordia command definitions and handlers live together in `scripts/primordia-command-handlers.ts`; `scripts/primordia.ts` instantiates them through `lib/tiny-command/cli.ts`, while `/api/core` instantiates the same definitions through `lib/tiny-command/rest.ts`. the in-app `/api/core` route-action surface exposes selected runnable CLI definitions as `primordia-core.v1` endpoints (for example `/api/core/status`, `/api/core/thread`, `/api/core/server/[threadId]/status`, `/api/core/server/[threadId]/logs`, `/api/core/thread/[threadId]/logs`, and `/api/core/thread/[threadId]/followup`) and runs those handlers in-process with an injected `CommandContext` rather than spawning a CLI subprocess per request; the ThreadView page is a lightweight client shell that loads thread metadata/preferences and the agent log from Core, with `/api/core/thread/[threadId]/logs` using visibility-aware `start` cursor resubscription, authenticated only by revokable `web` API keys from `/settings/api-keys`; first-party web clients automatically create a browser-scoped web API key after login for users with thread/evolve access, store it in localStorage, send it as a Bearer token to Core endpoints, retry once with a freshly-created key when the stored key is stale/revoked, and revoke it on logout; Core routes require both the logged-in session and a `web` API key for the same user; GET query parameters, POST JSON bodies, and multipart form bodies map to CLI args/options, log-style commands stream text when `follow=true` or NDJSON with `json=true`, and `/api-docs` renders the generated Core OpenAPI schema through Scalar. The shared Thread Request Form uses these Core thread endpoints for new threads and follow-ups, including attachments, preset selection, browser-held AES credential forwarding through the web API key, and caveman preference options. |
...
| OpenAPI spec | ✅ Live | Legacy app REST API spec still served at `/api/openapi`, generated on first request from `openapi-gen.config.json`; `/api-docs` now renders the Primordia Core route-action spec from `/api/core/openapi` through Scalar |
