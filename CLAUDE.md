...
| Hosting | Docker self-hosting or exe.dev | Docker Compose supports Raspberry Pi/self-hosting with persistent `/data`; exe.dev uses production builds via `bun run build && bun run start`; the reverse proxy manages both proxy and production app; blue/green slot swap on accept |
...
| Docker/Raspberry Pi self-hosting | ✅ Live | `docker compose up -d --build` runs Primordia with persistent runtime state in the `primordia-data` volume; Docker deploy accepts use an in-process typecheck/build/proxy-spawn path instead of systemd |
