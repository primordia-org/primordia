...
| Explicit preview target selection | ✅ Live | Agents set the session preview panel route by running `bun run set-preview-url /route`, which writes a structured `preview_path` event instead of relying on ambiguous final-message path parsing |
| Explicit preview target selection | ✅ Live | Agents set the session preview panel route by running `bun run set-preview-url /route`, which writes a structured `preview_path` event instead of relying on ambiguous final-message path parsing |
...
| Multiple agent harnesses | ✅ Live | Evolve form lets users choose harness (claude-code, pi, or codex) and model; preferences persisted per-user in DB; Pi evolve runs require the `npm:@agnishc/edb-todo` package and expose TaskCreate/TaskList/TaskGet/TaskUpdate/TaskOutput/TaskStop for task tracking; Evolve Session renders Pi TaskCreate/TaskList/TaskGet/TaskUpdate calls as To-do entries; Codex exec JSON is normalized into the same structured tool/reasoning session events as the other harnesses |
| Multiple agent harnesses | ✅ Live | Evolve form lets users choose harness (claude-code, pi, or codex) and model; preferences persisted per-user in DB; Pi evolve runs require the `npm:@agnishc/edb-todo` package and expose TaskCreate/TaskList/TaskGet/TaskUpdate/TaskOutput/TaskStop for staged task tracking (inspect/read → edit/implement → validate/check → wrap up; transient `.pi/tasks/` ignored); Codex exec JSON is normalized into the same structured tool/reasoning session events as the other harnesses |
| Multiple agent harnesses | ✅ Live | Evolve form lets users choose harness (claude-code, pi, or codex) and model; preferences persisted per-user in DB; Pi evolve runs require the `npm:@agnishc/edb-todo` package and expose TaskCreate/TaskList/TaskGet/TaskUpdate/TaskOutput/TaskStop for staged task tracking on every request (inspect/read → edit/implement → validate/check → wrap up; transient `.pi/tasks/` ignored); Codex exec JSON is normalized into the same structured tool/reasoning session events as the other harnesses |
| Multiple agent harnesses | ✅ Live | Evolve form lets users choose harness (claude-code, pi, or codex) and model; preferences persisted per-user in DB; Pi evolve runs require the `npm:@agnishc/edb-todo` package and expose TaskCreate/TaskList/TaskGet/TaskUpdate/TaskOutput/TaskStop for staged task tracking on every request (initial TaskCreate calls batched in one turn; inspect/read → edit/implement → validate/check → wrap up; transient `.pi/tasks/` ignored); Codex exec JSON is normalized into the same structured tool/reasoning session events as the other harnesses |
