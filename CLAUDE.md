...
| CLI framework | Internal tiny CLI helper (`lib/tiny-cli.ts`) | Organizes `bun run primordia` subcommands, detailed handwritten-style help, argument/option parsing, JSON automation output, generated bash completion with dynamic completion hooks, an injectable `ProcessCtx` for command handlers, and Primordia Core route-action metadata for exposing CLI commands through POST API endpoints |
