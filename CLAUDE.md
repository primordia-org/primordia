...
| Evolve CLI commands | ✅ Live | `bun run primordia new "request"` creates an evolve thread and runs the initial agent turn from the terminal; `bun run primordia agent [--thread <id>]` and `bun run primordia reply [--thread <id>] "request"` continue existing threads; compatibility aliases are available as `bun run primordia evolve create|run|followup`; CLI callers select users/presets/models with flags and pass the browser-held AES JWK through `PRIMORDIA_AES_KEY` when a stored billing source must be decrypted. |
