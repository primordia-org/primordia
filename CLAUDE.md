...
| Process status CLI | ✅ Live | `bun run process status` lists git worktrees, assigned ports, active Next.js server process state/env/PID, and active agent worker PIDs without querying the reverse proxy; JSON output exposes `servers` and `agents` arrays; supports table, `--json`, and `--watch` output |
