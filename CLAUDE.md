...
| Hosting | Docker self-hosting or exe.dev | Docker Compose supports Raspberry Pi/self-hosting with persistent `/data`; the lightweight image seeds the source and runs `scripts/install.sh` with mise instead of owning a custom runtime/deploy path; exe.dev uses production builds via `bun run build && bun run start`; the reverse proxy manages both proxy and production app; blue/green slot swap on accept |
...
| Docker/Raspberry Pi self-hosting | ✅ Live | `docker compose up -d --build` runs a lightweight Debian container that seeds Primordia into the persistent `primordia-data` volume, runs the normal `scripts/install.sh` + mise flow, and then starts the reverse proxy; accepted changes continue to use the installer path so the app can update dependencies and rebuild itself autonomously |
