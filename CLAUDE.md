...
| File attachments in evolve | ✅ Live | Attach images/files to initial and follow-up requests; files are copied into `worktree/attachments/` so Claude can read and use them; the page picker only highlights/captures nearest `data-component` elements with a single blue component outline |
...
| Multiple agent harnesses | ✅ Live | Evolve form lets users choose harness (claude-code, pi, or codex) and model; preferences persisted per-user in DB; Pi evolve runs require the `npm:@agnishc/edb-todo` package and expose TaskCreate/TaskList/TaskGet/TaskUpdate/TaskOutput/TaskStop for staged task tracking on every request (initial TaskCreate calls batched in one turn; compound task titles split into sub-tasks or smaller sibling tasks; TaskUpdate completion+next-start transitions batched in one turn; inspect/read → edit/implement → validate/check → wrap up; transient `.pi/tasks/` ignored); Evolve Session renders Pi TaskCreate/TaskList/TaskGet/TaskUpdate calls as task accordion entries with weighted progress bars, step tick marks, and nested text/reasoning/tool-call events (including a virtual Setup task for pre-list events and final-task grouping after completion); Codex exec JSON is normalized into the same structured tool/reasoning session events as the other harnesses |
