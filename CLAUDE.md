...
| Process status CLI | ✅ Live | `bun run process status` lists git worktrees, assigned ports, active Next.js server PID/mode, and active agent worker PIDs without querying the reverse proxy; supports table, `--json`, and `--watch` output |
