...
| Process status CLI | ✅ Live | `bun run process status` lists reverse proxy status plus git worktrees sorted by assigned port, active Next.js server process state/env/PID/child count, and active agent worker PIDs without querying the reverse proxy; JSON output exposes `reverseProxy`, `servers`, and `agents` arrays; supports table and `--json` output; `bun run process <worktreename> start [--dev|--prod]`, `stop`, and `restart [--dev|--prod]` manage assigned-port worktree servers directly and append stdout/stderr to `.primordia-next-server.log`; `bun run process <worktreename> logs [--follow] [--json]` reads that log; the evolve session Web Preview logs/restart controls use this process-manager layer instead of reverse-proxy preview endpoints |
