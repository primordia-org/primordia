...
| CLI framework | Internal tiny command framework (`lib/tiny-command/common.ts`, `lib/tiny-command/cli.ts`, `lib/tiny-command/rest.ts`; `lib/tiny-cli.ts` re-exports for compatibility) | Organizes `bun run primordia` subcommands, detailed handwritten-style help, argument/option parsing, JSON automation output, generated bash completion with dynamic completion hooks, an injectable `CommandContext` (`{ process, console }`) for command handlers, and Primordia Core route-action metadata/REST execution for exposing CLI-shaped commands through API endpoints |
