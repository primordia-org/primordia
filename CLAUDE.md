...
├── .gitignore
├── instrumentation.ts             ← Next.js instrumentation hook; reconnects/recover thread workers on server boot; scheduled jobs live behind the Primordia Core jobs boundary instead
...
├── lib/                           ← Shared utilities, DB adapter, auth helpers, scheduled-jobs boundary + lib/jobs implementations — see .claude/rules/filemap-lib.md
