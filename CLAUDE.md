...
| CLI framework | citty | Organizes `bun run primordia` subcommands, generated help, and argument definitions while preserving JSON automation output |
| AI code gen | `@anthropic-ai/claude-agent-sdk` | `query()` runs Claude Code in git worktrees for thread requests |
...
├── .gitignore
├── instrumentation.ts             ← Next.js instrumentation hook; starts update/audit schedulers and reconnects/recover thread workers on server boot
