...
| Thread CLI commands | ✅ Live | `bun run primordia create "request"` creates a thread and runs the initial agent turn from the terminal; `bun run primordia followup "request"` continues the thread identified by the current worktree; `bun run primordia update` applies parent/prod updates to the thread identified by the current worktree (or `--worktree`); `bun run primordia accept` and `bun run primordia reject` accept/deploy or discard the thread identified by the current worktree (or `--worktree`); CLI callers select users/presets with flags and pass the browser-held AES JWK through `PRIMORDIA_AES_KEY` when a stored billing source must be decrypted; `/settings/cli` shows the shell-quoted assignment for copying that key into a terminal; billing source, harness, model, evolve permission checks, and accept/reject behavior live in `lib/threads.ts` (`createThread`/`followupThread` accept preset IDs and resolve preset details internally; `manageThread` is shared by the API route and CLI) so the CLI never imports API route modules directly. |
| Thread CLI commands | ✅ Live | `bun run primordia create "request"` creates a thread from the terminal, starts setup/agent work in a detached background helper, and returns immediately with the session ID; `bun run primordia followup "request"` continues the thread identified by the current worktree and also returns immediately after the follow-up is accepted; CLI callers select users/presets with flags and pass the browser-held AES JWK through `PRIMORDIA_AES_KEY` when a stored billing source must be decrypted; `/settings/cli` shows the shell-quoted assignment for copying that key into a terminal; billing source, harness, model, and evolve permission checks live in `lib/threads.ts` (`createThread`/`followupThread` accept preset IDs and resolve preset details internally) so the CLI never imports API route modules directly. |
| Thread CLI commands | ✅ Live | `bun run primordia create "request"` creates a thread from the terminal, starts setup/agent work in a detached background helper, and returns immediately with the new worktree path for easy `cd`; `bun run primordia followup "request"` continues the thread identified by the current worktree and also returns immediately after the follow-up is accepted; CLI callers select users/presets with flags and pass the browser-held AES JWK through `PRIMORDIA_AES_KEY` when a stored billing source must be decrypted; `/settings/cli` shows the shell-quoted assignment for copying that key into a terminal; billing source, harness, model, and evolve permission checks live in `lib/threads.ts` (`createThread`/`followupThread` accept preset IDs and resolve preset details internally) so the CLI never imports API route modules directly. |
| Process status CLI | ✅ Live | `bun run primordia status` lists the git-configured production branch, reverse proxy status, git worktrees sorted by assigned port, active Next.js server process state/env/PID/child count, and active agent worker PIDs without querying the reverse proxy; JSON output exposes `productionBranch`, `reverseProxy`, `servers`, and `agents` arrays; supports table and `--json` output; `bun run primordia start [--dev|--prod] [--json] [--worktree <worktreename>]`, `stop [--json] [--worktree <worktreename>]`, and `restart [--dev|--prod] [--json] [--worktree <worktreename>]` manage assigned-port worktree servers directly and append stdout/stderr to `.primordia-next-server.log`; `bun run primordia logs [--follow] [--json] [--worktree <worktreename>]` reads that log; `bun run primordia publish [--json] [--worktree <worktreename>]` health-checks the target branch's assigned port before marking it as `primordia.productionBranch`; `bun run primordia copydb [--json] [--worktree <worktreename>]` VACUUM-copies the production SQLite DB into the target worktree, defaulting to the worktree containing the current directory; the evolve session Web Preview logs stream over SSE from `/api/evolve/server-logs`, and restart controls/reverse proxy lazy preview spawning use this process-manager layer instead of reverse-proxy-owned preview processes |
