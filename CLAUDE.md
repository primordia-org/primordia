...
| Hosting | exe.dev | Production builds via `bun run build && bun run start`; app scripts set `NEXT_DEPLOYMENT_ID` from the current git commit so Next.js skew protection refreshes stale clients across deploys; the Next CLI is invoked directly with `bun --bun ./node_modules/next/dist/bin/next ...` so long-running app servers do not fall through the package-bin `node` shebang; single systemd service (`primordia`) runs a small Primordia process supervisor, which keeps detached `reverse-proxy.js` and `scheduled-jobs.js` service processes alive without restarting them when the supervisor itself restarts; blue/green slot swap on accept |
