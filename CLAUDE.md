...
| CLI framework | Internal tiny CLI helper (`lib/tiny-cli.ts`) | Organizes `bun run primordia` subcommands, detailed handwritten-style help, argument/option parsing, JSON automation output, generated bash completion with dynamic completion hooks, and Primordia Core protocol metadata for exposing CLI commands over HTTP/WebSocket |
