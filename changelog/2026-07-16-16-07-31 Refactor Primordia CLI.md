...
Follow-up profiling found that completion is still too slow because `scripts/primordia.ts` statically imports `lib/threads.ts`, which pulls the pi coding agent and AI provider stack into even the `__complete` path. Added `docs/primordia-cli-completion-performance-proposal.md` with bundle-size findings and a proposed lazy-handler split so help/completion can load only tiny command metadata.
