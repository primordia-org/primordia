...
- Added `bun run progress` with `plan insert`, `plan replace`, and `step done|failed` commands that append `progress_*` events to the session NDJSON log.
...
- Ensured completed agent sections still render their progress panel even when the run ends successfully, errors, times out, or is aborted before any tool call.
