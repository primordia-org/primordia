...
- `lib/tiny-command/cli.ts` instantiates command definitions as a terminal CLI, creates the simple CLI context internally, and provides an `abortSignal` driven by process SIGINT/SIGTERM.
...
`scripts/primordia-command-handlers.ts` now owns both the Primordia command definitions and their handlers. `scripts/primordia.ts` only passes those definitions to the CLI runtime, while `app/api/core/[[...path]]/route.ts` only supplies Primordia-specific auth/cwd/OpenAPI configuration to the REST runtime.
