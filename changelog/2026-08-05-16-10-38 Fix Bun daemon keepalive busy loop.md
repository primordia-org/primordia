...
The new helper keeps daemon processes alive with a normal event-loop handle so they sleep correctly between real work. The `primordia jobs run` CLI path now uses the same safe keepalive behavior.
