...
Changed the user-facing thread detail URL from `/evolve/session/[id]` to `/thread/[id]` and removed the old route entirely so broken or stale links fail visibly instead of being masked by redirects.
