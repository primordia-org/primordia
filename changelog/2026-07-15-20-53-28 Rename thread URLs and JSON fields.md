...
Updated UI navigation, thread creation flows, diff/log streams, attachment links, admin-created thread flows, and internal preview DB hotswap calls to use the new route names. A final non-changelog sweep also renamed remaining user-facing "evolve" wording to thread terminology, including shared thread dialog/form component names, settings copy, admin permission copy, docs, tests, and route comments. Internal compatibility keys such as the existing `can_evolve` role and persisted preference/draft keys remain unchanged where renaming would require a migration.
