...
- Added `--user`, `--preset`, and `--auth-source` options; secret-backed billing sources use the caller-provided `PRIMORDIA_AES_KEY` environment variable.
- Removed direct CLI `--harness` and `--model` options so harness/model selection comes from presets, matching the web UI.
