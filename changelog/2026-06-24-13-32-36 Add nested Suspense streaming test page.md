...
Also hardened the evolve preview panel against simple self-recursion: if the iframe resolves to the same pathname as the session page itself, the panel shows a calm “Preview hidden to prevent infinitely nested previews” message with an eye-off icon and a “Show Anyway” override.

Follow-up: `bun install` failure messages now add a direct link to [Socket.dev status](https://status.socket.dev/) when the output looks like a temporary Socket.dev scanner outage, such as a 503/service-unavailable response. The hint explains that this is a blocker because Primordia prioritizes server safety, so the best course of action is to try again or wait until the security scanner is available again. The hint is shared by evolve setup, Apply Updates installs, local accept installs, and the production installer.
