...
- Standardized Core API multipart file fields on `attach` (no `attachment`/`attachments` aliases); short CLI aliases remain shell-only and are not accepted as Core API field/query aliases.
