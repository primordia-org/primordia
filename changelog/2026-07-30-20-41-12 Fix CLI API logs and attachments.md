...
- Made `primordia thread logs` render the session event stream as concise, human-readable console text when `--json` is not provided, coalescing adjacent text events inline while preserving natural whitespace and newlines from the original chunks.
