...
- Restored repeatable `--attach <file>` on `thread create` and `thread followup`.
- Standardized multipart file fields on `attach` (no `attachment`/`attachments` aliases), including the thread form and follow-up UI.
