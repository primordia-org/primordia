...
Added a dedicated copy button with success feedback aligned with the code, and restyled the prompt to match the cleaner Claude.ai credential form style. Replaced the spinner with plain helper text so the UI does not imply the user is waiting on an in-progress local action.
