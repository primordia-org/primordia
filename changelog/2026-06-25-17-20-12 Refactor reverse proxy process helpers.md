...
Scheduled operational jobs are now grouped behind `lib/scheduled-jobs.ts`. It starts Primordia update-source fetch checks, dependency audit checks, and disk cleanup. The reverse proxy remains the singleton that starts scheduled jobs for now, but only by calling `runScheduledJobs(...)`; Next.js instrumentation now only reconnects evolve workers after server restart.

Disk cleanup concerns moved out of the proxy/process-manager split: `lib/disk-space-management.ts` owns disk usage checks and worktree deletion/archive cleanup, while `lib/scheduled-jobs.ts` owns the recurring job timers.
