...
Scheduled operational jobs are now grouped behind `lib/scheduled-jobs.ts`. It starts Primordia update-source fetch checks, dependency audit checks, and disk cleanup. Disk cleanup now exposes `startDiskCleanupJobScheduler(...)` from `lib/disk-space-management.ts`, matching the update-source and dependency-audit scheduler APIs. The reverse proxy remains the singleton that starts scheduled jobs for now, but only by calling `runScheduledJobs(...)`; Next.js instrumentation now only reconnects evolve workers after server restart.

Disk cleanup concerns moved out of the proxy/process-manager split: `lib/disk-space-management.ts` owns disk usage checks, worktree deletion/archive cleanup, and its recurring job timer.
