...
The evolve session page now makes the web preview available as soon as an agent records an explicit preview path with `bun run set-preview-url`, instead of waiting for the agent run to reach the ready state. If an agent never records an explicit preview path, the preview still appears on the default session route once the session is ready. The preview server status and logs also start updating once the preview is available.
