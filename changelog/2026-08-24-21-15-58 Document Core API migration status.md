...
Follow-up: added an install-time workaround that prunes incompatible Linux libc native binary packages. Bun installs both glibc and musl optional packages for the Claude Agent SDK and Next.js SWC because it filters native optional dependencies by OS/CPU but not libc-specific package metadata/naming; the postinstall step removes the unused fallback so installs keep only the binary for the current runtime. The ideal long-term fix is for Bun/package-manager dependency resolution to choose the matching libc variant up front, at which point this cleanup hook should be removed.
