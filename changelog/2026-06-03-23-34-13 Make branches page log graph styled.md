...
Added `bun run export-branch-parentage-mermaid`, a small diagnostic tool that exports the same conceptual branch-parent DAG as a Mermaid `flowchart` so the parentage model can be inspected independently from the visual branch graph.
