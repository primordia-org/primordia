...
This keeps Pi/agent task progress and detailed execution history in one scannable accordion instead of splitting the to-do summary from the detailed log below it. The accordion is integrated directly into the agent card so it avoids an extra nested wrapper and redundant padding around the task progress UI.
