...
3. **TypeScript type check** — `bun run typecheck` (`tsc --noEmit`) is run in the session worktree. If any type errors are present, the accept is blocked and the compiler output is returned.
